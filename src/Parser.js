...
export function parse_fml_emdis(raw_string) {
    ...
    values_group.forEach(element => {
        ...
        emdis.message_type = message_type;
...
export function displayListOfDiffs(listDiffs) {
    ...
    listDiffs.forEach((element,idx) => {
        result += `differences in message ${idx} :\n`;
...
function displayDiff(emdisDiff) {
    var result = Object.keys(emdisDiff).reduce((accu, k) => accu += showSingleDiff(k, emdisDiff), "");
    return result;
}

function showSingleDiff(key, diff) {
